...
/**
 * Handles the HTTP POST request to update or create family medical history for a specific patient.
 ...
 */
export const updateStudentFamilyHistoryHandler = async (event, context) => {
	...
	try {
		...
		const transactionResult = await transaction(client, logger, async () => {
			...
			const currentDataResult = await client.query(
				`SELECT * FROM ${SCHEMA_NAME}.antecedentes_familiares WHERE id_paciente = $1`,
				[patientId],
			);
			logger.info(`Found ${currentDataResult.rowCount} patients...`);

			if (currentDataResult.rowCount > 0) {
				const { medicalHistory: existingData } = mapToAPIFamilyHistory(
			...
			const defaultValues = genDefaultFamiliarHistory();
			const values = [
				patientId,
				JSON.stringify(
		...
		if (upsertResult.rowCount === 0) {
			...
			return responseBuilder
				.setStatusCode(404)
				.setBody({ message: "Failed to insert or update family history." })
				.build();
		}

		logger.info("Mapping DB response into API response...");
		const updatedRecord = upsertResult.rows[0];
		const formattedResponse = mapToAPIFamilyHistory(updatedRecord);
