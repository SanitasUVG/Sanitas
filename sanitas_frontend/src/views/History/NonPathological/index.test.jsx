...
describe("NonPathologicalHistory Component Tests", () => {
  test("initial render and data fetching", async () => {
    mockGetNonPathologicalHistory.mockResolvedValue({
      result: {
        medicalHistory: {
          smoker: { data: [{ smokes: false, cigarettesPerDay: "0", years: "0" }], version: 1 },
          drink: { data: [{ drinks: false, drinksPerMonth: "0" }], version: 1 },
          drugs: { data: [{ usesDrugs: false, drugType: "", frequency: "" }], version: 1 },
        },
      },
    });

    mockGetBloodTypePatientInfo.mockImplementation(() => Promise.resolve({ bloodType: "O+" }));
    ...
    await waitFor(() => expect(screen.getByText("Antecedentes No Patológicos")).toBeInTheDocument());
  });
  ...
  test("displays error message when there is an error fetching data", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Ha ocurrido un error en la búsqueda, ¡Por favor vuelve a intentarlo!"),
      ).toBeInTheDocument()
    );
  });
});
