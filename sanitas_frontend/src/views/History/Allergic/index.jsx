 ...
 */
function AllergicView({ id, birthdayResource, allergicHistoryResource, updateAllergicHistory }) {
    ...
    const noAllergicData = !AllergicHistory.data || AllergicHistory.data.length === 0;
    ...
    const handleOpenNewForm = () => {
        setSelectedAllergie({ selectedMed: "", whichAllergie: "", reactionType: "" });
        setAddingNew(true);
    };

    // Save the new Allergic record to the database
    const handleSaveNewAllergie = async () => {
        if (!selectedAllergie.selectedMed || !selectedAllergie.whichAllergie || !selectedAllergie.reactionType) {
        ...
        const updatedCategory = {
            version: 1,
            data: [...(AllergicHistory[selectedAllergie.selectedMed]?.data || []), {
    ...
    const handleFieldChange = (fieldName, value) => {
        setSelectedAllergie((prevAllergie) => ({ ...prevAllergie, [fieldName]: value }));
    };
        ...
        >
            ...
            >
                ...
                {noAllergicData && !errorMessage
                    ...
                    : (
                        Object.keys(allergicHistoryData?.medicalHistory || {}).map((category, index) => {
                            return allergicHistoryData.medicalHistory[category].data.map((allergy, idx) => (
                                <InformationCard
                                    key={`${index}-${idx}`}
                                    type="allergy"
