...
import { formatDate, calculateYearsBetween } from "src/utils/date";
...
function DiagnosisSection({
  ...
  const [diagnosisName, setDiagnosisName] = useState(diagnosisDetails.illness || "");
  const [medication, setMedication] = useState(diagnosisDetails.medication || "");
  ...
  }, [diagnosed, diagnosisName, medication, dose, frequency]);
...
function OperationSection({
  ...
  const checkPerformed = (resource) => (Array.isArray(resource) ? resource.length > 0 : !!resource);

  const [performed, setPerformed] = useState(() => checkPerformed(operationDetailsResource));
  ...
  const canAddMore = () => {
    if (operationKey === "breastMassResection" || operationKey === "ovarianCysts") {
  ...
  if (birthYearResult.error) {
    const error = birthYearResult.error;
    if (error?.response) {
...
function ObGynView({ id, birthdayResource, obgynHistoryResource, updateObGynHistory }) {
  ...
  const {
    ...
    diagnosedIllnesses = {
      data: {
        ovarianCysts: { medication: { dosage: "", frequency: "", medication: "" } },
        uterineMyomatosis: { medication: { dosage: "", frequency: "", medication: "" } },
        endometriosis: { medication: { dosage: "", frequency: "", medication: "" } },
        otherCondition: { medication: { dosage: "", frequency: "", medication: "", illness: "" } },
      },
    },
    hasSurgeries = {
      data: {
  ...
  const [age, setAge] = useState(
    firstMenstrualPeriod.data.age != null ? firstMenstrualPeriod.data.age.toString() : "",
  );
  ...
  const [isPainful, setIsPainful] = useState(
    painfulMenstruation.data.isPainful != null ? painfulMenstruation.data.isPainful : false,
  );

  const [medication, setMedication] = useState(
    painfulMenstruation.data.medication != null ? painfulMenstruation.data.medication : "",
  );
  ...
  const [A, setA] = useState(pregnancies.data.abortions != null ? pregnancies.data.abortions : 0); // Abortos
  ...
  const [diagnoses, setDiagnoses] = useState(() => {
    ...
    (diagnosedIllnesses.data.otherCondition ?? []).forEach((condition) => {
  ...
  const getDiagnosisDetails = (key) => {
    ...
    return condition && condition.medication
      ? {
        ...
        if (diagnosis.key === diagnosisKey) {
          if (!isActive) {
            ...
            const updatedDetails = {
              medication: newDetails.medication || diagnosis.details.medication || "",
              dosage: newDetails.dosage || diagnosis.details.dosage || "",
              frequency: newDetails.frequency || diagnosis.details.frequency || "",
            };

            if (diagnosis.isNew && diagnosis.key.startsWith("Nuevo Diagnóstico")) {
              updatedDetails.illness = newDetails.illness || diagnosis.details.illness || "";
            }
      ...
      prevOperations.map((operation) => {
        if (operation.key === operationKey) {
          const clearedDetails = isPerformed
            ? operation.details
            : operation.key === "ovarianCysts" || operation.key === "breastMassResection"
              ? []
              : {};
  ...
  const validateGynecologicalHistory = () => {
    ...
    if (invalidDiagnosis) {
      toast.error("Por favor, complete los detalles de todos los diagnósticos activos.");
      return false;
    }

    const invalidOperation = operations.some((operation) => {
      const details = Array.isArray(operation.details) ? operation.details : [operation.details];
  ...
  const fixedDiagnosesKeys = ["ovarianCysts", "uterineMyomatosis", "endometriosis"];
  ...
  const handleSaveGynecologicalHistory = async () => {
    ...
    if (!result.error) {
      ...
      toast.error(`Error al actualizar los antecedentes ginecoobstétricos: ${result.error}`);
    }
  };
            ...
            {operations.map((operation, index) => (
              <div key={operation.key}>
                <OperationSection
                  ...
                  editable={true}
                  birthdayResource={birthdayResource}
                  operationDetailsResource={mapOperationDetails(operation.key)}
