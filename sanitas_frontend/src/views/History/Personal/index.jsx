...
export function PersonalHistory({
  ...
  const LoadingView = () => {
    return <Throbber loadingMessage="Cargando información de los antecedentes personales..." />;
  };
 ...
 */
function PersonalView({ id, birthdayResource, personalHistoryResource, updatePersonalHistory }) {
  ...
  const [personalHistory, setPersonalHistory] = useState({
    ...
    diabetesMellitus: {
      ...
      version: personalHistoryData?.medicalHistory.diabetesMellitus.version || 1,
    },
    hypothyroidism: {
    ...
    myocardialInfarction: {
      ...
      version: personalHistoryData?.medicalHistory.myocardialInfarction.version || 1,
    },
    cancer: {
  ...
  const noPersonalData = Object.keys(personalHistory).every(
    (key) => personalHistory[key] && personalHistory[key].data && personalHistory[key].data.length === 0,
  );
  ...
  const handleSaveNewPersonal = async () => {
    if (!selectedPersonal.disease || !personalHistory[selectedPersonal.disease]) {
      toast.error("Por favor, selecciona una enfermedad válida.");
      return;
    }
    ...
    const updatedData = [...personalHistory[selectedPersonal.disease].data, newEntry];
    ...
    >
      ...
      >
        ...
        {noPersonalData && !errorMessage
          ...
          : (
            Object.entries(personalHistory).map(([diseaseKey, { data = [], version }]) => {
              if (data.length === 0) {
                return null;
              }
                ...
                {selectedPersonal.disease === "cancer" && (
                  <React.Fragment>
                    ...
                    <BaseInput
                      ...
                      onChange={(e) => setSelectedPersonal({ ...selectedPersonal, treatment: e.target.value })}
                      readOnly={!addingNew}
                  ...
                  && (
                    <React.Fragment>
                      ...
                      <BaseInput
                        ...
                        onChange={(e) => setSelectedPersonal({ ...selectedPersonal, medicine: e.target.value })}
                        readOnly={!addingNew}
                        placeholder="Ingrese el tratamiento administrado"
                        style={{
                          width: "95%",
                      ...
                      <BaseInput
                        ...
                        onChange={(e) => setSelectedPersonal({ ...selectedPersonal, dose: e.target.value })}
                        readOnly={!addingNew}
                      ...
                      <BaseInput
                        ...
                        onChange={(e) => setSelectedPersonal({ ...selectedPersonal, frequency: e.target.value })}
                        readOnly={!addingNew}
