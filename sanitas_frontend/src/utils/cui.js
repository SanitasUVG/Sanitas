...
export function cuiIsValid(cui) {
	...
	const sanitizedCui = cui.replaceAll(/\s/g, "");
	const depto = Number.parseInt(sanitizedCui.substring(9, 11), 10);
	const muni = Number.parseInt(sanitizedCui.substring(11, 13), 10);
	const numero = sanitizedCui.substring(0, 8);
	const verificador = Number.parseInt(sanitizedCui.substring(8, 9), 10);
	...
	if (modulo !== verificador) {
		return { error: `${modulo} is different from ${verificador}` };
	}
	return true;
}
